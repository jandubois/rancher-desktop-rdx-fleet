...
Every extension image must contain a `metadata.json` file at the root of its filesystem that defines the [contents of the extension](./extensions-sdk/extensions/architecture/metadata.md).

The Docker image must have several [image labels](./extensions-sdk/extensions/DISTRIBUTION/labels.md), providing information about the extension. See how to use [extension labels](./extensions-sdk/extensions/DISTRIBUTION/labels.md) to provide extension overview information.
...
Extensions that install binaries on the host must also provide Windows binaries in the same extension image. See how to [build a multi-arch image](./extensions-sdk/extensions/DISTRIBUTION/multi-arch.md) for your extension.
