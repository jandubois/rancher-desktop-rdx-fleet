---
description: Here you will learn how to use the key applications
lang: en
robots: index, follow
title: Reference guides \| SUSE Application Collection
twitter:card: summary
twitter:description: Here you will learn how to use the key applications
twitter:title: Reference guides
viewport: width=device-width,initial-scale=1,shrink-to-fit=no
---


# Reference guides

##### Here you will learn how to use the key applications

Before reading these articles, we suggest following the [Get started](get-started) section.


------------------------------------------------------------------------


### [Apache Ant](reference-guides/apache-ant.md)

###### Learn how to install and use Apache Ant


### [Apache APISIX](reference-guides/apache-apisix.md)

###### Learn how to install and use Apache APISIX


### [Apache Kafka](reference-guides/apache-kafka.md)

###### Learn how to install and use Apache Kafka


### [ClamAV](reference-guides/clamav.md)

###### Learn how to install and use ClamAV


### [External Secrets Operator](reference-guides/external-secrets-operator.md)

###### Learn how to install and use External Secrets Operator


### [ExternalDNS](reference-guides/external-dns.md)

###### Learn how to install and use ExternalDNS


### [Go FIPS](reference-guides/go-fips.md)

###### Learn how to install and use Go FIPS


### [InfluxDB](reference-guides/influxdb.md)

###### Learn how to install and use InfluxDB


### [Istio](reference-guides/istio.md)

###### Learn how to install and use Istio


### [Kiali](reference-guides/kiali.md)

###### Learn how to install and use Kiali


### [Local Path Provisioner](reference-guides/local-path-provisioner.md)

###### Learn how to install and use Local Path Provisioner


### [MariaDB](reference-guides/mariadb.md)

###### Learn how to install and use MariaDB


### [MinIO](reference-guides/minio.md)

###### Learn how to install and use MinIO


### [NATS](reference-guides/nats.md)

###### Learn how to install and use NATS


### [OpenTelemetry Collector](reference-guides/opentelemetry-collector.md)

###### Learn how to install and use OpenTelemetry Collector


### [OpenTelemetry Operator for Kubernetes](reference-guides/opentelemetry-operator.md)

###### Learn how to install and use OpenTelemetry Operator for Kubernetes


### [Penpot](reference-guides/penpot.md)

###### Learn how to install and use Penpot


### [PostgreSQL](reference-guides/postgresql.md)

###### Learn how to install and use PostgreSQL


### [Velero](reference-guides/velero.md)

###### Learn how to install and use Velero


Last modified July 8, 2025


