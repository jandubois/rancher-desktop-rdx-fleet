---
description: SUSE Application Collection documentation site
generator: Hugo 0.147.8
lang: en
robots: index, follow
title: SUSE Application Collection
twitter:card: summary
twitter:description: SUSE Application Collection documentation site
twitter:title: Documentation
viewport: width=device-width,initial-scale=1,shrink-to-fit=no
---


# Documentation

##### SUSE Application Collection documentation site

Welcome to the SUSE Application Collection documentation site!

Here you will familiarize with the web application and learn how to start using the applications.


------------------------------------------------------------------------


### [Get started](get-started.md)

###### This section of the documentation contains a set of articles that will help you understand how the Application Collection works and how to start using it


### [How-to guides](howto-guides.md)

###### In this part of the documentation site, you will find *How-to* guides for using the Application Collection


### [Reference guides](reference-guides.md)

###### Here you will learn how to use the key applications



Last modified September 9, 2025


